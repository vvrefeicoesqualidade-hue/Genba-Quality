...
.comment-wrap { margin: -2px 16px 10px; }
</style>
    ...
    try:
        ...
        for c in conf_rows:
            ...
            historico.append({
                ...
                "respostas": respostas, "comentarios": comentarios, "total_c": total_c,
...
def salvar_conferencia(conf, respostas, comentarios=None):
    try:
        ...
        ws_resp.append_rows([[f"RS{conf_id}{i:03d}", conf_id, "", v, comentarios.get(it, ""), it, conf["turno"], conf["local"]]
        ...
        if not ncs:
            ...
            for h, it in ncs:
                ...
                st.markdown(
                    ...
                    f'<div><div class="nc-name">{it}</div><div class="nc-meta">{h["local"]} · {h["turno"]} · {h["data"]}</div></div>'
