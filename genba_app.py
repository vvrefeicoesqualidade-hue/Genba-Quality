...
/* COMMENT WRAP (bottom part of the item card) */
.comment-wrap {
    background: #fff;
    border: 0.5px solid #D6EDD9;
    border-top: none;
...
.comment-wrap.c  { border-color: #1D9E75; }
.comment-wrap.nc { border-color: #E24B4A; }
...
.nc-comment { font-size: 12px; color: #6B2020; margin-top: 6px; background: #fff; border-radius: 8px; padding: 6px 8px; }
...
def item_key(item):
    """Chave estável para widgets, baseada no texto do item."""
    return str(abs(hash(item)) % (10**8))

# ── Session state ─────────────────────────────────────────────
for k, v in [("tela","inicio"),("respostas",{}),("comentarios",{}),("conf",{}),("historico",[]),("fiscal","Fiscal")]:
...
if st.session_state.tela == "inicio":
                ...
                <div class="hist-card">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;">
                        <div>
                            ...
                            {''.join([f'<div style="font-size:11px;color:#9C0006;margin-top:3px;">• {n}{(" — " + coment.get(n)) if coment.get(n) else ""}</div>' for n in nc_list]) if nc_list else ''}
                        </div>
    ...
    with tab3:
        ...
        if not ncs:
            ...
            for h, it in ncs:
                coment = h.get('comentarios', {}).get(it, "")
                st.markdown(f"""
                <div class="nc-card">
                    ...
                    {f'<div class="nc-comment">💬 {coment}</div>' if coment else ''}
                </div>""", unsafe_allow_html=True)
...
# ═══════════════════════════════════════════════════════════════
    ...
    for item in itens:
        ...
        cls = "c" if status == "C" else "nc" if status == "NC" else ""
        row_cls = f"item-row {cls}".strip()
        ...
        with c1:
            ...
            if st.button("✔ Conforme", key=f"c_{item_key(item)}", use_container_width=True):
                st.session_state.respostas[item] = "C"; st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
        with c2:
            st.markdown('<div class="btn-nc">', unsafe_allow_html=True)
            if st.button("✖ Não Conforme", key=f"nc_{item_key(item)}", use_container_width=True):
                st.session_state.respostas[item] = "NC"; st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
        with c3:
            st.markdown('<div class="btn-clear">', unsafe_allow_html=True)
            if st.button("↺", key=f"r_{item_key(item)}", use_container_width=True):
                st.session_state.respostas[item] = None; st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)

        # Campo de comentário (opcional) para este item
        comment_cls = f"comment-wrap {cls}".strip()
        st.markdown(f'<div class="{comment_cls}">', unsafe_allow_html=True)
        comentario = st.text_area(
            "Comentário (opcional)",
            value=st.session_state.comentarios.get(item, ""),
            key=f"com_{item_key(item)}",
            placeholder="Adicione uma observação, se necessário...",
            height=68,
            label_visibility="visible",
        )
        st.session_state.comentarios[item] = comentario
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    # Botão extra: enviar conferência completa (com todos os comentários) via WhatsApp
    tem_comentarios = any(c.strip() for c in coment.values())
    if avaliados > 0:
        linhas = []
        for it, v in resp.items():
            if v is None:
                continue
            marca = "✅" if v == "C" else "❌"
            linha = f"{marca} {it}"
            if coment.get(it, "").strip():
                linha += f"\n   💬 {coment[it]}"
            linhas.append(linha)
        itens_txt = "\n".join(linhas)
        msg_full = (f"🌿 *Conferência — Genba Quality Analytics*\n"
                    f"📍 {conf['local']} · {conf['turno']}\n"
                    f"📅 {conf['data']}\n"
                    f"👤 Fiscal: {conf['fiscal']}\n\n"
                    f"{itens_txt}\n\n"
                    f"✅ C: {total_c}  ❌ NC: {total_nc}  📊 {pct}% conformidade")
        wa_url_full = f"https://wa.me/?text={urllib.parse.quote(msg_full)}"
        st.markdown(f"""
        <div class="wa-btn-wrap">
            <a href="{wa_url_full}" target="_blank" class="wa-link">
                📲 Enviar conferência completa via WhatsApp
            </a>
        </div>""", unsafe_allow_html=True)
    ...
    with col1:
        ...
        if st.button("✓ Finalizar", use_container_width=True):
            st.session_state.historico.append({
                **conf, "respostas": dict(resp), "comentarios": dict(coment),
